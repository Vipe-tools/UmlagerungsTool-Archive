...
def process_and_print(event=None):
    ...
    if stellplatz:
        for sp in stellplatz.split():
            if not re.fullmatch(r"[A-Z]{2,3}-\d{1,4}", sp):
                messagebox.showerror("Fehler", f"Ungültiger Stellplatz: {sp}\nErlaubt: 2–3 Buchstaben, dann '-', dann 1–4 Zahlen.")
                return
        ws["D37"] = stellplatz
...
def clear_fields():
    ...
    zusatz_status_label.config(text="Aktuell: kein Zusatz")
...
Stellplatz: falls vorhanden, sonst leer lassen. Mehrere Stellplätze möglich, z.B. 'DD-09 BB-34'.

Datum: wird automatisch mit dem heutigen Datum gefüllt (kann geändert werden).

Zusatz: optionaler Kommentar zu Artikel.
...
🧩 Erstellt von Nicklas Aue – Version 1.11 (11.12.2025)"""
    messagebox.showinfo("Hilfe", help_text)

def show_update_info():
    update_text = """🟦 Update-Info

Version 1.11 (11.12.2025):

- Neuer Info zu Artikel Button, hiermit kann man sich Daten zu dem Artikel anzeigen lassen
- Info zu Artikel Button kann auch mit F11 geöffnet werden
- Optionale Funktion Zusatz: hier kann man einfach irgendeinen Kommentar hinzufügen
- Die optionale Funktion Stellplatz akzeptiert nur noch gültige Stellplätze und keinen beliebigen Text """
...
root = tk.Tk()
root.title("UmlagerungsTool v1.11")
