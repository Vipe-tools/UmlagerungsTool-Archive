...
# Basis-Ordner bestimmen (für .pyw und .exe)
...
font_file = os.path.join(base_folder, "arial.ttf")  # Arial.ttf muss eingebunden oder daneben liegen
...
def search_excel_file():
    ...
    for f in os.listdir(excel_folder):
        if f.lower().endswith(".xlsx") and search_text in f.lower():
            ...
            break
    if found_file:
        excel_path.set(os.path.basename(found_file))
    else:
        select_excel_file()
...
def select_excel_file():
    ...
    if file_path:
        excel_path.set(os.path.basename(file_path))
...
root = tk.Tk()
root.title("Umlagerungen zu Amazon v1.5")
