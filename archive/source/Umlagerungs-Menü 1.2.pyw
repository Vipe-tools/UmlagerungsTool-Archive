...
font_file = os.path.join(base_folder, "arial.ttf")  # <- Schriftartdatei einfügen
...
def generate_barcode_from_text():
...
root = tk.Tk()
root.title("Umlagerungen zu Amazon v1.2")
...
tk.Button(root, text="Barcode generieren", command=generate_barcode_from_text).grid(row=2, column=2, padx=5)
