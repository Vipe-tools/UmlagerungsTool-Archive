...
def show_help(event=None):
    long_help_text = """🟨 Anleitung – Version 2.0.0
...
🧩 Version 2.0.0 (0562026)
"""
    show_long_text("Hilfe", long_help_text)

def show_update_info():
    long_update_text = """
Neu: Eigene Artikel erstellen und bearbeiten

Mit der neuen Artikelverwaltung können Sie nun Ihre eigenen Artikel anlegen, bearbeiten und verwalten. Alle wichtigen Informationen zu Ihren Artikeln lassen sich bequem an einem Ort pflegen.

Viel Spaß mit der neuen Artikelverwaltung!
...
def show_info(event=None):
    try:
        programm_pfad = os.path.join(os.path.dirname(sys.executable if getattr(sys, 'frozen', False) else __file__), "Artikelverwaltung.exe")

        if not os.path.isfile(programm_pfad):
            messagebox.showerror("Fehler", "artikelverwaltung.pyw nicht gefunden.")
            return

        os.startfile(programm_pfad)

    except Exception as e:
        messagebox.showerror("Fehler", str(e))

root = tk.Tk()
root.title("UmlagerungsTool v2.0.0")
...
tk.Button(root, text="Artikelverwaltung", command=show_info, bg="lightblue").grid(row=7, column=1, padx=(140,5), sticky="w")
